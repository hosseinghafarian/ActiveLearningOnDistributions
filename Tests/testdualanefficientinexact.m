...
s = sdpvar(1);
cConstraint = [X>=0,p>=0,a>=0,g>=0,A_E*Xr==b_ECt,A_I*Xr==s,s<=s_IC,A_EV*Xr==b_EV];%
...
cObjective = -b_ECt'*y_E+ 1/2*norm(A_E'*y_E+A_I'*y_I+A_EV'*y_EV+DuXr+Gr)^2+1/2*norm(gprox+v-y_I)^2+v*s_IC;%
sol = optimize(cConstraint,cObjective);
if sol.problem==0 
   Sd = value(S);
   vd = value(v);
