function docode
% this is a test for dual version of the problem, i.e. obtain primal
% variables from dual variables. It works but it doesnot have B_EV and
% B_IV, so the next version is testdualanefficientinexact7.m
close all
recomp=1;
y = rand(10,1);
...
cObjective = -(b_EC'*y_EC+b_EV'*y_EV)+ arho/2*norm(A_EC'*y_EC+A_IC'*y_IC+A_EV'*y_EV+A_IV'*y_IV+DuXr+c_k+Gr)^2 ...
               +arho/2*norm(v+y_I-gprox)^2+v'*s_I-1/2*norm(Gr,'fro')^2-1/2*norm(gprox)^2;% ...
               %+arho/2*norm([y_IC;y_IV]-[AICdup;AIVdup])^2;%
sol = optimize(cConstraint,cObjective);
if sol.problem==0 
   ...
   Xgr    = c_k+Gr+value(DuXr)+A_EC'*y_ECd/scaleA_EC+A_IC'*y_ICd/scaleA_IC+A_EV'*y_EVd/scaleA_EV+A_IV'*y_IVd/scaleA_IV;
   Xgr2    = c_k+Gr+value(DuXr)+A_EC'*AECdup/scaleA_EC+A_IC'*y_ICd/scaleA_IC+A_EV'*AEVdup/scaleA_EV+A_IV'*y_IVd/scaleA_IV;
   norm(Xgr-Xgr2)
