function docode
% it is working when there is no inequality constraints
close all
recomp=1;
y = rand(10,1);
load('data4test','lambda','K','KE','alphapre','alphatild','nSDP','n_S','nSDP','n_l','n_u','initL','unlabeled','query','Yl','batchSize','n_o','n_lbn');
%load('data4test','lambda','KE','alphapre','alphatild','nSDP','n_S','nSDP','n_l','n_u','initL','unlabeled','query','Yl','batchSize','n_o','n_lbn');
...
% coefficients for g which substituted by p+q so, it is considered for q
...
Gr  = [zeros(nSDP*nSDP,1);zeros(n_S,1)]; 
...
s2 = sdpvar(n_IV,1);
s  = [s1;s2];
% cConstraint = [X>=0,p>=0,A_EC*Xr==b_EC,A_IC*Xr==s1,A_IV*Xr==s2,A_EV*Xr==b_EV,s1<=s_IC,s2<=s_IV];%,X(nSDP,query)>=0,X(nSDP,query)>=0
% cObjective  = -c_k'*Xr+1/2*norm(Xr-Gr)^2+1/2*norm(s-gprox)^2;
% cConstraint = [X>=0,p>=0,A_EC*Xr==b_EC,A_EV*Xr==b_EV];%,X(nSDP,query)>=0,X(nSDP,query)>=0
% cObjective  = -c_k'*Xr+1/2*norm(Xr-Gr)^2;%+1/2*norm(s-gprox)^2;
cConstraint = [X>=0,p>=0,A_EC*Xr==b_EC,A_EV*Xr==b_EV];%,A_IC*Xr==s1,s1<=s_IC];%,X(nSDP,query)>=0,X(nSDP,query)>=0
cObjective  = -c_k'*Xr+1/2*norm(Xr-Gr)^2;%+1/2*norm(s-gprox)^2;
if recomp==1
...
%         AIVdup = dual(cConstraint(5));
        AEVdup = -dual(cConstraint(4));% negative because of the direction of the constraint
...
% cConstraint= [ S>=0,pdu>=0,v>=0];%,v<=arho*(s_I-gprox)+y_I];%,Z(1:n_S,:)==0,Z(:,1:n_S)==0,Z(n_S+1:nSDP,n_S+1:nSDP)>=0];%,Z(nSDP,query)>=0];
% cObjective = -(b_EC'*y_EC+b_EV'*y_EV)+ arho/2*norm(A_EC'*y_EC+A_IC'*y_IC+A_EV'*y_EV+A_IV'*y_IV+DuXr+c_k+Gr)^2+arho/2*norm(v+y_I-gprox)^2+v'*s_I-1/2*norm(Gr,'fro')^2-1/2*norm(gprox)^2;%
cConstraint= [ S>=0,pdu>=0,v>=0];%,Z(1:n_S,:)==0,Z(:,1:n_S)==0,Z(n_S+1:nSDP,n_S+1:nSDP)>=0];%,Z(nSDP,query)>=0];
cObjective = -(b_EC'*y_EC+b_EV'*y_EV)+ 1/2*norm(A_EC'*y_EC+A_EV'*y_EV+DuXr+c_k+Gr)^2;%+1/2*norm(gprox+v-y_I)^2+v'*s_I-1/2*norm(Gr,'fro')^2-1/2*norm(gprox)^2;%
sol = optimize(cConstraint,cObjective);
if sol.problem==0 
   ...
   %Xgr    = Gr+value(DuXr)+A_EC'*y_ECd/scaleA_EC+A_IC'*y_ICd/scaleA_IC+A_EV'*y_EVd/scaleA_EV+A_IV'*y_IVd/scaleA_IV;
   Xgr    = c_k+value(DuXr)+A_EC'*y_ECd/scaleA_EC+A_EV'*y_EVd/scaleA_EV;
   feasAEC= norm(A_EC*Xgr-b_EC);
   fvAEC  = A_EC*Xgr-b_EC;
   fvAEV  = A_EV*Xgr-b_EV;
   fvAIC  = max(-s_IC+A_IC*Xgr,0);
   fvAIV  = max(-s_IV+A_IV*Xgr,0);
