function docode
% test if removing A_IV and A_IC resolves the problem when there is no w_o
close all
recomp=0;
...
cObjective = -(b_EC'*y_EC+b_EV'*y_EV)+ arho/2*norm(A_EC'*y_EC+A_EV'*y_EV+DuXr+c_k+Gr)^2;%+arho/2*norm(v+y_I-gprox)^2;%+v'*s_I-1/2*norm(Gr,'fro')^2-1/2*norm(gprox)^2;%
...
if sol.problem==0 
   ...
   Xgr    = Gr+value(DuXr)+A_EC'*y_ECd/scaleA_EC+A_EV'*y_EVd/scaleA_EV;
