...
%              next
function query_ind =uncertainty_sampling(xTrain, yTrain, initL, samples_to_query_from)

  classes  =[-1 ;1 ];
  s= 0.1;
  k = 7;
  
  probabilities = knn_model(classes,xTrain, yTrain,initL,k,s);
  
  scores = marginal_entropy(probabilities);

  [~,ind] = max(scores(samples_to_query_from));
  query_ind = samples_to_query_from(ind);
