function [query_id] = vari_logis_LOCGLOB_PMAL(trainsamples, initL, samples_to_query_from, K, lambda, learningparams, alparams) 
global cnstData
   ...
   [queryind] = vlocgloblog_PMAL( Lind, Uind, y_Lindex, alphavar );
   ...
   function [queryind] = vlocgloblog_PMAL(Lind, Uind, y_Lindex, alphavar )


      n_u = numel(Uind);
      mlogpyx_yq_yu = zeros(n_u, 1);
      for q = 1:n_u
          [mlogpyx_yq_yu(q), prexi] = comp_maxyq_locglob(K, Lind, q, Uind, y_Lindex, alphavar, prexi);
      end
      [~, indq] = min(mlogpyx_yq_yu);
      queryind = Uind(indq);
   end
    function [val, prexi] = comp_maxyq_locglob(K, Lind, q, Uind, y_Lindex, alphavar, prexi)
        qind          = Uind(q);
        Lindq         = [Lind, qind];
        ...
        [val1 , xi1] = comp_maxyq_minyu_locglob(K, Lindq, Uindmiq, y_Lindexplusq, alphavar, prexi);
        
        y_q        = -1;
        y_Lindexplusq = [y_Lindex, y_q];
        [valm1 , xim1] = comp_maxyq_minyu_locglob(K, Lindq, Uindmiq, y_Lindexplusq, alphavar, prexi);
    ...
    function [val , xi] = comp_maxyq_minyu_locglob(K, Lindq, Uindmiq, y_Lq, alphavar, prexi)
       [y, xi] = compute_y_opt(K, Lindq, Uindmiq, y_Lq, alphavar, prexi);
       %%%%Warning: the following objective function doesn't consider
       %%%%ln\vert S_n\vert in the objective function. It may be
       %%%%inaccurate. 
       val =  1/8*y'*Mxi*y + sum( log(logisticfunc(xi)) - 0.5* xi + 0.5*(logisticfunc(xi)-0.5).*xi);     
    end
    function [y_opt, xi] = compute_y_opt(K, Lindq, Uindmiq, y_Lq, alphavar, prexi)
       tol = 0.00001;
       conv = false;
       y = zeros(size(K,1), 1);
       while ~conv
           Mxi = computeL(K, prexi, alphavar);
           %y_U = inv(L(Uindmiq, Uindmiq))*L(Uindmiq, Lindq)*y_Lq;
