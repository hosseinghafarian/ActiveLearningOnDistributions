function [model] = SMM3waytrain(learningparams, data, idx, useidxkernel )
global cnstDefs
if nargin==3
    useidxkernel = true;
end
...
%     distu        = unique(data.F); % what are the unique distnums 
    ...
    if ~useidxkernel
        K            = data.K_3way;% Here we must update the kernel matrix if kernel params changed = data.K(idx,idx);
    else
        K            = data.K_3way(idx,idx);% Here we must update the kernel matrix if kernel params changed = data.K(idx,idx);
    end
