function [query, acc_ML, risks] = DISTgaussian_harmonic_semisupervised_activelearning(nqueries, Y, W, initL, samples_to_query_from)
...
for iteration=1:nqueries
    ...
    RISK                 = realmax*ones(n,1);
    RISK(U)              = risk;
	[minrisk, minUindex] = min(RISK(samples_to_query_from));

	query(iteration) = samples_to_query_from(minUindex);
