function [ALresult,model_set,Y_set]= ActiveOutlierSaddle12(warmStart,x_primalwmst,y_dualwmst,ModelAndData,learningparams)
...
%% Get Constraints
global operators
[operators,lo,up] = getConstraints3(learningparams);      
%% Initialize values of dual (lagrange) variables : if there is no previous values for them
if warmStart
    ...
    load('variablebetw','beta_curr','w_obetapre','X','p','a','g','u');
    x_G.u       = upre;
    x_G.st       = stpre;
    x_G.w_obeta = w_obetapre;
    alpha_alpha0      = beta_curr;
end
%% starting values for proximal update, if we have warmstart it must be updated to previous point


%% Setting ADMM parameters
thao           = 1;%1.618;% Best values for these parameters. Donot increase mul. , if these values changes in any iteration we must recompute cholesky factors below
rhoml          = 1/(learningparams.lambda_o*learningparams.rho+1);
%% initialize time and convergence measure variables
timeprof       = zeros(100,4);
convergemeasure= zeros(100,3);
%% Loading just for checking result
load('datafile.mat','alphav','G_plusv','pv','qv','inpsolverobjective');
%% Computing Lipschitz parameters
L_f                      = computeLipSchitz(learningparams);
optparams.Lipconstalpha  = L_f;%10; % this must be update, it is the L_xx+\frac{L_xy^2}{\lambda} lipschitz for f(x,alpha), be aware that it is half the value of L
%% Starting Proximal Loop
tst             = cputime;
max_iter        = optparams.stmax_iter; % but proximal iteration also exists whenever convergence measure become shrinkfac times of pre convergence measure.
...
while outerproxit < optparams.maxouterprox 
    ...
    [x_next,beta_curr,dualvars,timeprofile(outerproxit,:)] ...
                = ProximalStepAlphaX(x_G,alpha_alpha0,dualvarsPre, optparams,learningparams, lo,up);
    ...
    disp(outputstr);
    if i>max_iter && ~converged % it is stalled, no progress in one Iteration
        
    end
    %% Update Previous Lagrange Values
...
function [x_curr,beta_curr,dualvars,timereport] = ProximalStepAlphaX(x_G,alpha_alpha0,dualvarsPre, optparams,learningparams, lo,up)
    global cnstData
    verbose = true;
    %% Setting Parameters
    alphamaxit    = 1000;
    ...
    while ~converged && (i<=max_iter)
        ...
        proxtime = tic;        
        [x_new, dualvars, f_alpha,gradf_alpha,...
            etallstart,etallend,timeDetailproxfi,iterSDP] = proxf_X_directADMM(learningparams, optparams, dualvarsPre,...                                                                 %pre primalvalues for SDP,nonpositivity constraints
                                                                               x_G,alphak); 
        profiler(1);
        maxobju   = f_alpha;
        ...
        wmstalpha = beta_curr;
        tic;
        [v_k, iterAlpha,minobjalpha]   = argminpsi(wmstalpha,alpha0,accumGrad,accumf,accumgrada,learningparams.rho,A_curr,lo,up,optparams.alphatol,alphamaxit);
    ...
    function updateandprintconvmeasure(verbose)
        ...
        if verbose
            ...
            str=sprintf('%4.0d |%7.4f|%7.4f   |%7.4f|%7.4f |%7.4f|%7.4f|%7.4f|%7.4f|%7.4f |%3.0d  |%3.0d|%4.3f|%9.6f|%8.5f|%8.5f',...
                i,convergemeasure(i,7),convergemeasure(i,1),convergemeasure(i,6),convergemeasure(i,2),convergemeasure(i,3),convergemeasure(i,4),convergemeasure(i,5),etallstart,etallend,...
                iterSDP,iterAlpha,ptoEnd,primalobj-dualobj,primalobj,dualobj);
            disp(str);
        end
    end
    function profiler(type)
        if type ==1
            ...
            timeinSDPproj = timeinSDPproj + timeDetailproxfi.SDPprojtime;
            t1 = t1  + timeDetailproxfi.t1all;
            t2 = t2  + timeDetailproxfi.t2all;
            t3 = t3  + timeDetailproxfi.t3all;
        elseif type == 2
...
function [ x_next, dualvars, f_alpha,gradf_alpha,...
            etallstart,etallend,timeDetailproxf,iter] = proxf_X_directADMM(learningparams, optparams, dualvarsPre,...                                                                 %pre primalvalues for SDP,nonpositivity constraints
                                                                       x_G,alpha_k)
...
global operators
global cnstData
[c_k]  = computecoeff(learningparams.lambda,learningparams.ca,alpha_k,cnstData.KE,cnstData.nSDP,cnstData.n_S,cnstData.extendInd,cnstData.unlabeled);
useyalmip   = 0;                                                        
if useyalmip==1
...
function [operators,lo,up] = getConstraints3(learningparams)
global cnstData
...
lo        = [zeros(cnstData.n_S,1);-ones(cnstData.nSDP-1-cnstData.n_S,1)];
up        = ones(cnstData.nSDP-1,1);
...
function [c_k]=computecoeff(lambda,c_a,alphapre,KE,nSDP,n_S,query,unlabeled)
...
cq        = alphatild(unlabeled)+c_a*ones(size(unlabeled,1),1);
...
global cnstData
    [c_k]    = computecoeff(learningparams.lambda,c_a,alpha_k,cnstData.KE,cnstData.nSDP,cnstData.n_S,cnstData.extendInd,cnstData.unlabeled);
