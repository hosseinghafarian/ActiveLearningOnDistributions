function [ f_ls_x, grad_ls_x] = f_ls_substitute_of_constraints(x_k, operators )
%% This function is least square substitute for constraints. 
%       It has two types in the first form rho = rho_EC and 
%           f(x)= \rho/2 \Vert A*x.u+B*x.w_obeta-[b_E;x.st]\Vert^2
%                        in the second form 
...
f_type      = 1; rho_EC = 10; rho_EV = 10; rho_IC = 10; rho_IV = 10;
...
if f_type == 1
    ...
    f_ls_x = rho_x/2* norm(operators.A*x_k.u + operators.B*x_k.w_obeta-[b_E;x_k.st])^2;
    grad_ls_x.u       = rho_x*operators.A'*(operators.A*x_k.u + operators.B*x_k.w_obeta-[b_E;x_k.st]);
    grad_ls_x.w_obeta = rho_x*operators.B'*(operators.A*x_k.u + operators.B*x_k.w_obeta-[b_E;x_k.st]);
    grad_ls_x.st      = rho_x*(x_k.st-operators.A_I*x_k.u - operators.B_I*x_k.w_obeta); 
else % if f_type == 2 
