...
global cnstData 
    ...
    if retType == 1 || retType ==2 % y_E 
        ...
        RHS           = b_E - operators.A_E*(operators.A_I'*y_I + c_SZ)...
                            - operators.B_E*(cnstData.Hinv*operators.B_I'*y_I +z_hat.w_obeta);
        if retType ==1             % pcg
            Mat           = operators.LHS_E;
        else% retType ==2          % chol
    ...
    elseif retType == 3 || retType == 4 % y_I
        ...
        RHS           = z_hat.st-v-operators.A_I*(operators.A_E'*y_E+c_SZ)...
                                  -operators.B_I*(cnstData.Hinv*operators.B_E'*y_E +z_hat.w_obeta);
        if retType == 3            % pcg
            Mat           = operators.LHS_I;
        else% retType == 4         % chol
    ...
    [f_dual] = dual_regwo_objective_split(y_EC, y_EV, y_IC, y_IV, S, Z , v, z_hat, operators, learningparams, optparams);
end