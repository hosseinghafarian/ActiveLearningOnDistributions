function [model] = RSVM_OutlierAblation(learningparams, data, idx) 
...
%         ht             = 1-solve.opt_vars.p;
        model.h        = value(h_eta);
        model.name     = 'RSVM_OutlirAblation';
    else
        model = struct([]);
    end
    function [solve] = doOptimize(Options)
        ...
        if solve.solproblem == 0    
            [solve.x_opt, solve.opt_vars] = extractOptResults(cObjective, learningparams, H, h_eta, beta_p, eta_p, Options);
    ...
    function [cConstraint, cObjective]    = constraintType(options)
        ...
        MR           = 0;%learningparams.lambda_alpha_D_part*M1 + learningparams.lambda_alpha_Q_part*M2;
        KVMatrix     = [G.*H(1:nap,1:nap)+ MR ,h_eta+eta_p-beta_p;(h_eta+eta_p-beta_p)',2*t/lambda];        
...
%         cConstraint  = [cConstraint, (h_eta(1:n_S)            == g_D_pos(1:n_S) + g_D_neg(1:n_S)):'abs_h'];
        cConstraint  = [cConstraint, (h_eta(1:n_S) >=0):'h_positive'];
        cConstraint  = [cConstraint, (h_eta(1:n_S) <=1):'h_lessthan1'];
        cConstraint  = [cConstraint, (H(1:n_S,nap+1)      == h_eta(1:n_S)):'value_h'];
...
%         cConstraint  = [cConstraint, (g_D_pos(initL) - g_D_neg(initL) == c_o*Yl'-K_o(initL,:)*w_o):'label_h'];
        ...
        cConstraint  = [cConstraint, (diag(H(initL,initL))== h_eta(initL)):'diag_h'];
%         cConstraint  = [cConstraint, (h_eta                   == Gamma *(c_o*ones(n_S,1)-Yl'.*(K_o*w_o))):'diagwo_h'];
...
%            end
%         end
        cObjective = t + sum(beta_p) -sum(h_eta);%+ sum(eta_p(extndind));% + learningparams.ca*sum(h_eta(unlabeled));
...
function [x_opt, opt_vars] = extractOptResults(cObjective, learningparams, H, h, beta_p, eta_p, Options)
