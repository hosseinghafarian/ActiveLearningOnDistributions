function [ALresult, model_set, Y_set]= ActiveDConvexRelaxOutlierCPBased(WMST_beforeStart, Model, learningparams, xTrain, yTrain)
% In this function we use a model with norm one penalty: learningparams.cp*\Vert
% \Psi(X)^\mathsf(T) w_o\Vert_1^2 
        global cnstData
...
%%      Define YALMIP Variables        
        ...
        r       = sdpvar(n,1);        % For Filtering out y_ui : absolute value of y_ui (1-y_ui w_o(\phi(x_i)) 
        ...
        h       = sdpvar(nap,1);
        rl      = sdpvar(n_l,1);
        ONED    = [ones(n,1);zeros(cnstData.n_q,1)];
        u       = [reshape(G_plus,cnstData.nSDP*cnstData.nSDP,1);p];
%%      Define Problem, Constraints and Objective  
        % options for making constraints
        Options.constraintForm = 5;    Options.addsumqobjective = false;
...
%             model_set.noisySamplesInd = noisysamplesInd;
    ...
    function [cConstraint, cObjective]    = constraintType(addsumqobjective,useq, diagunlabIneq,diagsetQueryIneq)
        ...
        if useq 
            ...
            cConstraint= [cConstraint, r(query)+gamma(query).*q+gamma(query).*p(query)==gamma(query)]; 
            if numel(unlabNotQ)>0 
                cConstraint= [cConstraint, r(unlabNotQ)+gamma(unlabNotQ).*p(unlabNotQ)==gamma(unlabNotQ)]; 
            end
            ...
            cConstraint= [cConstraint, r(query)+gamma(query).*G_plus(extndind,nap+1)+gamma(query).*p(query)==gamma(query)];
            if diagsetQueryIneq 
        ...
        if diagunlabIneq
            cConstraint= [cConstraint, diag(G_plus(setunlab,setunlab))<=r(setunlab) ];
        else
            cConstraint= [cConstraint, diag(G_plus(setunlab,setunlab))==r(setunlab) ];
        end
        cConstraint= [cConstraint, g_D(initL)==p(initL) ];
        cConstraint= [cConstraint, g_D(extndind)==zeros(cnstData.n_q,1)];
        cConstraint= [cConstraint, g_D(setunlab)==1-r(setunlab)];
        ...
        cConstraint= [cConstraint, r(setunlab)>=G_plus(setunlab,nap+1), r(setunlab)>=-G_plus(setunlab,nap+1)]; % r is for absolute value of y_u.*(1-pu).*(1-q)
        
        cObjective = t+lambda_o*w_o'*cnstData.K_o*w_o/2+sum(beta_p)+sum(eta_p(extndind))+learningparams.ca*sum(r(setunlab)-hbar);
...
%             [noisysamplesInd] = get_noisy_sample_ind(pv, n));
...
%             noisysamplesInd   = noisysamplesInd(isunlabelednoisy);
    ...
    function [solproblem, cObjectivev, x_opt, G_plusv, qv, pv,beta_pv,eta_pv, misc ] = doOptimize(Options)
        global cnstDefs
        ...
        if Options.useoperators
            ...
            [cConstraint, cObjective] = constraintType(Options.addsumqobjective,Options.useq, Options.diagunlabIneq,Options.diagsetQueryIneq);
        end
        ...
        if solproblem == 0
            ...
            [x_opt,G_plusv,qv,pv,beta_pv,eta_pv,misc] ...
                   = extractOptResults(cObjective, learningparams, q, G_plus, p, w_o, g_D, h, r, rl, beta_p, eta_p, s1, s2, Options);
...
function [x_opt,G_plusv,qv,pv,beta_pv,eta_pv,misc ] = extractOptResults(cObjective, learningparams, q, G_plus, p, w_o, g_D, h, r, rl, beta_p, eta_p, s1, s2, Options)
    global cnstData
    %% Select Samples to query from : this code not written for part of unlabeled data are being queried from. 
    % Select All of unlabeled samples for Querying
    nap      = cnstData.nap;
    ...
    [x_opt] = x_conv(G_plusv,pv,w_ov,st);
end
