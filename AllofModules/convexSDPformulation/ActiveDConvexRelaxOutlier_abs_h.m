function [ALresult, model_set, Y_set]= ActiveDConvexRelaxOutlier_abs_h(WMST_beforeStart, Model, learningparams, xTrain, yTrain)
        global cnstData
...
%             assert(abs(fxoptalpha-fxoptalpha2)/abs(fxoptalpha2)<tolobj,'Objective function values doesnot match');
            ...
            abs_wox            = abs(model_set.w_oxT_i)';
            normdiffp          = norm(abs_wox-pv);
            model_set.G        = G_plusv;
    ...
    function [cConstraint, cObjective]    = constraintType(addsumpConstraint, addsumpobjective, addsumqobjective,useq, diagunlabIneq,diagsetQueryIneq)
        ...
        MR           = lambda_p1*M1 + lambda_p2*M2;
        KVMatrix   = [cnstData.KE.*G_plus(1:nap,1:nap)+lambda_alpha*eye(nap)+MR ,h+eta_p-beta_p;(h+eta_p-beta_p)',2*t/lambda];
    ...
    function [solproblem, cObjectivev, x_opt, G_plusv, qv, pv,beta_pv,eta_pv, misc ] = doOptimize(Options)
        ...
        if solproblem == 0
            ...
            [x_opt,G_plusv,qv,pv,beta_pv,eta_pv,misc] ...
                   = extractOptResults(cObjective, learningparams, q, G_plus, p, w_o, g_D_pos, g_D_neg, h, beta_p, eta_p, s1, s2, Options);
...
function [x_opt,G_plusv,qv,pv,beta_pv,eta_pv,misc ] = extractOptResults(cObjective, learningparams, q, G_plus, p, w_o, g_D_pos, g_D_neg, h, beta_p, eta_p, s1, s2, Options)
    global cnstData
    %% Select Samples to query from : this code not written for part of unlabeled data are being queried from. 
    % Select All of unlabeled samples for Querying
    nap      = cnstData.nap;
    ...
    [x_opt] = x_conv_abs_h(G_plusv, pv, g_D_neg, w_ov,st);
end
