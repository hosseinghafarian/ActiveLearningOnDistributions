function [alpha_k, x_k, dualvars_k, solstatus ] = adjoint_lssdp_ABCD_dual_noscale(x_G, alpha_0, dualvars_0, operators,optparams,learningparams, progress_func,verbose, max_conv_meas, max_rel_gap, max_iter)
% Global variables 
global cnstData
global optimals
...
L_h       = cnstData.L_KE/learningparams.lambda;
...
noprogres_duration = 5;    % check if no progress has been made in the last noprogres_duration iterations
...
n_x_st        = numel(y_ICtil) + numel(y_IVtil);
...
while ~converged && iter < max_iter && progressing
    %% Step 1
%     [alpha_k]        = proxf_alpha(learningparams, optparams, alpha_ktil, x_k);%for test
%     [x_0 ] = update_x_0_alpha(alpha_k,x_G);%for test
    [ y_EC, y_EV, y_IC, y_IV, S, Z , v, normg_y_E, normg_y_I] = BCD_dual_solver{sub_solvetype}();%solve_dualsub_problems(sub_solvetype);    
    [x_k ]           = x_conv_from_dual_fullproject(y_EC,y_EV,y_IC,y_IV,S,Z,v, x_0, operators);
    %[alpha_k]        = proxf_alpha(learningparams, optparams, alpha_0, x_k);
    [alpha_k]        = proxf_alpha(learningparams, optparams, alpha_ktil, x_k);%for test
...
%This is not necessary, since x_k is the last x_k and it is correct, x_k                        = x_conv_from_dual_fullproject(y_EC, y_EV, y_IC, y_IV, S, Z, v, x_0, operators);
...
return
    ...
    function [y_EC, y_EV, y_IC, y_IV, S, Z , v, normg_y_E, normg_y_I ] = solve_BCDDetail_dualsub_problems
        ...
        [y_IC, y_IV] = proxLmu_y_I(objectivefunc,soltype4y_E_y_I,tol4y_E_y_I,maxit4y_E_y_I,...
                                       ...
                                       x_0, operators, learningparams,optparams);
        [normg_y_E] = get_norm_g_y_E(b_E,y_EC, y_EV, y_IC, y_IV, S, Z , v, x_0, operators, learningparams, optparams);
        [y_EC, y_EV] = proxLmu_y_E(objectivefunc,soltype4y_E_y_I,tol4y_E_y_I,maxit4y_E_y_I,...
                                       y_EC, y_EV, y_IC, y_IV, S, Z , v,...
                                       x_0, operators, learningparams,optparams);
        [normg_y_I] = get_norm_g_y_I(b_E,y_EC, y_EV, y_IC, y_IV, S, Z , v, x_0, operators, learningparams, optparams);                       
    end 
...
%         g_acc_x.st      = zeros(n_x_st,1);
        [f,g_acc_x,g_alpha]  = f_xAlpha_grad(x_G,alpha_k,learningparams);
    ...
    function [alpha_new]  = first_order_proxf_alpha(learningparams, optparams, alpha0, x_k)
        ...
        rhop      = learningparams.rhoalpha;%0.01;%1.5;%
        h_k       = 1/(optparams.L_alpha);
        [l_of_x, G_of_x] = x_obj_get_lG_of_x(x_k);
        KG        = (cnstData.KE.* G_of_x(1:nap,1:nap)) / learningparams.lambda;
