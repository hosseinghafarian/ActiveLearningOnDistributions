function [alpha_k, x_k, dualvars_k, solstatus ] = nonsaddle_lssdp_nestcomp_dual_noscale(x_G, alpha_0, dualvars_0, operators,learningparams,optparams, progress_func,verbose)
...
y_ECtil       = zeros(size(y_ECtil)); y_EVtil = zeros(size(y_EVtil)); y_ICtil = zeors(size(y_ICtil)); y_IVtil = zeros(size(y_IVtil)); Stil = zeros(size(Stil)); Ztil = zeros(size(Ztil)); vtil= zeros(size(vtil));
...
return
    ...
    function [y_EC, y_EV, y_IC, y_IV, S, Z , v, normg_y_E, normg_y_I, alpha_k ] = solve_dualsub_problems(typeofsolve)
             if typeofsolve == 1
             ...
             [x_k ]           = x_conv_from_dual_fullproject(y_EC,y_EV,y_IC,y_IV,S,Z,v, x_0, operators);
             [alpha_k, f_x]  = proxf_alpha(learningparams, optparams, alpha_0, x_k);
             [f,g_x,g_alpha]  = f_xAlpha_grad(x_k,alpha_ktil,learningparams);
             %alpha_k          = alpha_ktil  + mu_alpha*g_alpha;
    end
