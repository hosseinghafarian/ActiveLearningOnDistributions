function [x_avg,u_avg, solstatus] = saddle_solver_ver3(options, x_0, u_0, function_arguments, n_x, n_u, L_xx, L_uu, L_xu, max_conv_meas, max_rel_gap, max_iter)
...
u_proximal_operator = function_arguments.u_prox_operator;
x_proximal_operator = function_arguments.x_prox_operator;
...
while ~converged && (iter <= max_iter)
    [grad_x, grad_u, Hessian_xu ] = semi_second_order(x_k_tild, u_k); 
    if ~usexHessian && ~useuHessian 
       if ~useuL_xu 
          A_u     = Hessian_xu'*1/L_xx*Hessian_xu + L_uu*eye(n_u);
       end
       a_u        = -(grad_u-Hessian_xu'*(1/L_xx)*grad_x);  
       [u_k]      = u_proximal_operator(L_uu, L_xu, L_A_u, a_u, A_u, x_k_tild, u_k, useuL_xu);
       [grad_x, grad_u, Hessian_xu ] = semi_second_order(x_k_tild, u_k); 
       normgx = norm(grad_x);
       normgu = norm(grad_u);
       if ~usexL_xu 
       ...
       [x_k]      = x_proximal_operator(L_xx, L_xu, L_A_x, a_x, A_x, x_k, u_k, usexL_xu);
       u_k_tild   = u_k + theta_u*(u_k-u_kpre);
       [grad_x, grad_u, Hessian_xu ] = semi_second_order(x_k, u_k_tild); 
       normgx2 = norm(grad_x);
       normgu2 = norm(grad_u);
       if ~useuL_xu 
          A_u     = Hessian_xu'*1/L_xx*Hessian_xu + L_uu*eye(n_u);
       ...
       a_u        = -(grad_u-Hessian_xu'*(1/L_xx)*grad_x);  
       [u_k]      = u_proximal_operator(L_uu, L_xu, L_A_u, a_u, A_u, x_k, u_k, useuL_xu);
    ...
    x_avg           = x_sum /iter;
    u_avg           = u_sum /iter;
    objective(iter) = objectivefunc(x_avgpre, u_avgpre);
    max_u_obj       = objectivefunc(x_avgpre, u_k);
    min_x_obj       = objectivefunc(x_k     , u_avgpre);
    gap(iter)       = max_u_obj-min_x_obj;% max_u_obj must be greater than min_x_obj except for the first iteration. 
    if gap(iter) < mingap, mingap = abs(gap(iter)); end
    
    etaall(iter)  = measurefunc(meastype, x_avg, u_avg, x_avgpre, u_avgpre, iter);
    if etaall(iter) <= max_conv_meas && abs(gap(iter)) <= max_rel_gap
        converged = true;
    end
    x_k_tild      = x_k + theta_x*(x_k-x_kpre);
    x_kpre        = x_k;
    u_kpre        = u_k;
