function [v_k_x,dualvars]=solvePsi_k(x_k,operators,dualvars,...
...
c_u       = accumGrad_u;
c_beta    = accumGrad_beta;
c_s       = accumGrad_s;
zhat_u    = (1/gamma_reg)*(c_u    + arho*x_G.u      + mu_reg*x_k.u);
zhat_beta = (1/gamma_reg)*(c_beta + arho*x_G.w_obeta+ mu_reg*x_k.w_obeta);
zhat_st   = (1/gamma_reg)*(c_s    + arho*x_G.st     + mu_reg*x_k.st);
zhat.u    = zhat_u;
zhat.w_obeta = zhat_beta;
zhat.st   = zhat_st;
...
[y_EC,y_EV,y_IC,y_IV,S] = loaddualvars(dualvars);
recomp=1;
...
normc_k   = norm(zhat_u,'fro');
Gprox     = zhat_u;
gprox     = zhat_st;
gw_obetat = zhat_beta;

gammanorm = 1+sqrt(max(norm(Gprox),norm(gprox)));
Gprox     = Gprox /gammanorm;
gprox     = gprox /gammanorm;
gw_obetat  = gw_obetat/gammanorm;

Ghat.u    = Gprox;
Ghat.w_obeta = gw_obetat;
Ghat.st   = gprox;

b_ECbar   = b_EC/gammanorm;
b_EVbar   = b_EV/gammanorm;
s_IC      = s_IC/gammanorm;
s_IV      = s_IV/gammanorm;

maxCscale = normc_k;%max(normC_kM,normcp);
scaleA_EC = 1;
scaleA_EV = 1;
scaleA_IC = 1;
scaleA_IV = 1;
...
s           = [s1;s2];
tgamma      = 1;%gamma_reg;
const1      = 0.5*arho*compNorm(x_G,Q)+0.5*mu_reg*compNorm(x_k,Q);
cConstraint = [X>=0,p>=0,A_EC*Xr==b_ECbar,A_IC*Xr==s1,A_IV*Xr==s2+B_IV'*w_obeta,A_EV*Xr==b_EVbar+B_EV'*w_obeta,s1<=s_IC,s2<=s_IV, ...
               X(query,nSDP)>=0, X(nSDP,query)>=0 ];
cObjective  = tgamma/2*norm(Xr-Gprox)^2+tgamma/2*norm(s-gprox)^2+tgamma/2*(w_obeta-gw_obetat)'*Q*(w_obeta-gw_obetat);
if recomp==1
    ...
    if sol.problem == 0 
        ...
        AICdup = -dual(cConstraint(4)); % negative because of the direction of the constraint
        AIVdup = -dual(cConstraint(5)); % negative because of the direction of the constraint
        AEVdup = -dual(cConstraint(6)); % negative because of the direction of the constraint
        vdup   = -[dual(cConstraint(7));dual(cConstraint(8))];
...
% za   = sdpvar(n_S,1);
% zg   = sdpvar(n_S,1);
const2    = tgamma/2* compNorm(Ghat,Q);
% const2     = arho/2*compNorm(x_G,Q)+mu_reg/2*compNorm(x_k,Q);
Zpartq = sdpvar(n_u,1);
zapp = [zeros(n_S,1);Zpartq];
ZMat = [zeros(nSDP-1,nSDP-1),zapp;zapp',0];
...
% cObjective = -(b_ECbar'*y_EC+b_EVbar'*y_EV)+ 1/(2*gamma_reg)*norm(Aysdp+S+Z+gamma_reg*Gprox)^2 ...
%              + 1/2*(Bysdp+gamma_reg*zhat_beta)'*Qinv*(Bysdp+gamma_reg*zhat_beta)...
%              +1/(2*gamma_reg)*norm(-v+y_I-gamma_reg*gprox)^2-v'*s_I+const2;
cObjective = (b_ECbar'*y_EC+b_EVbar'*y_EV)- 1/(2*tgamma)*norm(Aysdp+S+Z+tgamma*Gprox)^2 ...
             - 1/(2*tgamma)*(Bysdp+tgamma*gw_obetat)'*Qinv*(Bysdp+tgamma*gw_obetat)...
             -1/(2*tgamma)*norm(v+y_I-tgamma*gprox)^2-v'*s_I+const2;
cObjective = - cObjective;
sol = optimize(cConstraint,cObjective);
if sol.problem==0 
   dobj = value(cObjective);
   objequal = abs(dobj+pobj);
   Sd = value(S);
   vd = value(v);
   Zd     = value(Z);
   y_ICd  = value(y_IC);
   ...
   Xgr    = Gprox+1/arho*(value(S)+A_EC'*y_ECd/scaleA_EC+A_IC'*y_ICd/scaleA_IC ...
                           +A_EV'*y_EVd/scaleA_EV+A_IV'*y_IVd/scaleA_IV);
   w_obetav = value(KQinv*(B_EV*y_EV+B_IV*y_IV+arho*Q*w_obetat));
   fvAEC  = A_EC*Xgr-b_ECbar;
   fvAEV  = A_EV*Xgr-b_EVbar+B_EV'*w_obetav;
   fvAIC  = max(-s_IC+A_IC*Xgr,0);
   fvAIV  = max(-s_IV+A_IV*Xgr+B_IV'*w_obetav,0);
   feasAEC= norm(A_EC*Xgr-b_ECbar);
   feasAIV= norm(max(-s_IV+A_IV*Xgr+B_IV'*w_obetav,0));
   feasAEV= norm(A_EV*Xgr+B_EV'*w_obetav-b_EVbar);
   ...
   xeq  = norm(Xg-Xp,'fro')/norm(Xp,'fro');
   dualequalprimalme= orth+norm(pd-pp)+objequal+xeq;
   feasDualAns      = feasAEC+feasAIV+feasAEV+feasAIC;
   
   sum(Xp(nSDP,19:34)*gammanorm)
   sum(Xg(nSDP,19:34)*gammanorm)
   u_tplus   = gammanorm*Xgr;
   w_obetavt = gammanorm*w_obetav;
   s_tplus   = gammanorm*(gprox-1/arho*(vd+value(y_I)));
else
    u_tplus   = Xgr;
end
epsvalue = 0.01;
assert(abs(pobj+dobj)/(1+abs(pobj))<epsvalue);
v_k_x.u         = u_tplus;
v_k_x.st        = s_tplus;
v_k_x.w_obeta   = w_obetavt; 
dualvars.y_EC   = y_ECd;
dualvars.y_EV   = y_EVd;
dualvars.y_IC   = y_ICd;
dualvars.y_IV   = y_IVd;
