function [u_tplus, w_obetavt, s_tplus, c_k]=testdualanefficientinexact9(arho,n_l,n_u,n_S,nSDP,batchSize,n_o,n_lbn,lambda,lambda_o,...
                                                            initL,unlabeled,query,...
                                                            A_EC,b_EC,A_IC,s_IC,A_EV,b_EV,A_IV,s_IV,B_EV,B_IV,y_EC,y_IC,y_EV,y_IV,E_E,E_I,...
                                                            u_t,s_t,w_obetat,...
                                                            Yl,K,KE,alphapre,c_k,Q)
...
gammanorm = max(norm(u_t+c_k/arho),norm(s_t));
...
s2 = sdpvar(n_IV,1);
s  = [s1;s2];
cConstraint = [X>=0,p>=0,A_EC*Xr==b_EC,A_IC*Xr==s1,A_IV*Xr==s2+B_IV'*w_obeta,A_EV*Xr==b_EV+B_EV'*w_obeta,s1<=s_IC,s2<=s_IV, ...
...
cObjective = -(b_EC'*y_EC+b_EV'*y_EV) ...
             + 1/(2*arho)*norm(A_EC'*y_EC+A_IC'*y_IC+A_EV'*y_EV+A_IV'*y_IV+S+Z+c_k+arho*u_t)^2 ...
             +1/2*(B_EV*y_EV+B_IV*y_IV+arho*Q*w_obetat)'*KQinv*(B_EV*y_EV+B_IV*y_IV+arho*Q*w_obetat)-lambda_o/2*w_obetat'*K*w_obetat ...
             +1/(2*arho)*norm(-v+y_I-arho*s_t)^2-v'*s_I-1/(2*arho)*norm(u_t,'fro')^2-1/(2*arho)*norm(s_t)^2;
...
%    norm(Xgr-Xgr2)
   ...
   feasAEV= norm(A_EV*Xgr+B_EV'*w_obetav-b_EV);
...
%    norm(value(pdu)-pdup)
   ...
   u_tplus   = Xgr;
   w_obetavt = w_obetav;
   s_tplus   = s_t-1/arho*(vd+value(y_I));
else
    u_tplus   = Xgr;
end
end
