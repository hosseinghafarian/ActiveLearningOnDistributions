function [x_next,dualvars]=testdualanefficientinexact10(learningparams,operators,dualvars,...
                                                            x_G,alphapre,c_k)
...
cConstraint = [X>=0,p>=0,A_EC*Xr==b_ECbar,A_IC*Xr==s1,A_IV*Xr==s2+B_IV'*w_obeta,A_EV*Xr==b_EVbar+B_EV'*w_obeta,s1<=s_IC,s2<=s_IV, ...
               X(query,nSDP)>=0, X(nSDP,query)>=0 ];%,X(nSDP,query)>=0,X(nSDP,query)>=0
cObjective  = arho/2*norm(Xr-Gprox)^2+arho/2*norm(s-gprox)^2 ...
              +lambda_o/2*w_obeta'*K*w_obeta+arho/2*(w_obeta-w_obetat)'*Q*(w_obeta-w_obetat);%-1/(2*arho)*norm(c_k)^2-c_k'*u_t
if recomp==1
...
% cObjective = -(b_ECbar'*y_EC+b_EVbar'*y_EV) ...
%              + 1/(2*arho)*norm(A_EC'*y_EC+A_IC'*y_IC+A_EV'*y_EV+A_IV'*y_IV+S+Z+arho*Gprox)^2 ...
%              +1/2*(B_EV*y_EV+B_IV*y_IV+arho*Q*w_obetat)'*KQinv*(B_EV*y_EV+B_IV*y_IV+arho*Q*w_obetat)-lambda_o/2*w_obetat'*K*w_obetat ...
%              +1/(2*arho)*norm(-v+y_I-arho*gprox)^2-v'*s_I-1/(2*arho)*norm(Gprox,'fro')^2-1/(2*arho)*norm(gprox)^2;
cObjective = arho/(2)*norm(1/arho*(A_EC'*y_EC+A_IC'*y_IC+A_EV'*y_EV+A_IV'*y_IV+S+Z)+Gprox)^2 -arho/(2)*norm(Gprox,'fro')^2 ...
             -(b_ECbar'*y_EC+b_EVbar'*y_EV)+ 1/2*(B_EV*y_EV+B_IV*y_IV+arho*Q*w_obetat)'*KQinv*(B_EV*y_EV+B_IV*y_IV+arho*Q*w_obetat)...
             -lambda_o/2*w_obetat'*K*w_obetat ...
             +1/(2*arho)*norm(-v+y_I-arho*gprox)^2-v'*s_I-arho/(2)*norm(gprox)^2;
...
epsvalue = 0.01;
assert(abs(pobj+dobj)<epsvalue);
x_next.u         = u_tplus;
x_next.st        = s_tplus;
x_next.w_obeta   = w_obetavt; 
dualvars.y_EC   = y_ECd;
dualvars.y_EV   = y_EVd;
dualvars.y_IC   = y_ICd;
dualvars.y_IV   = y_IVd;
